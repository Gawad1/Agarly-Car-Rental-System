...
const ReservationsReport: React.FC = () => {
  ...
  const [reportType, setReportType] = useState('ReservationsCar'); // Default report type
  const [reservationData, setReservationData] = useState<Reservation[]>([]);
  const [loading, setLoading] = useState(false);
  const [plate_id, setCarPlate] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/resPerCar', {
        ...
        body: JSON.stringify({ reportType, startDate, endDate, plate_id }),
      });
